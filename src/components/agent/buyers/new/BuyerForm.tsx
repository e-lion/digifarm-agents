...
import { createBuyer, getBuyerTypes, getValueChains, getContactDesignations } from '@/lib/actions/buyers'
...
export function BuyerForm() {
    ...
    const { data: buyerTypes = [] } = useQuery({ queryKey: ['buyerTypes'], queryFn: getBuyerTypes })
    const { data: valueChains = [] } = useQuery({ queryKey: ['valueChains'], queryFn: getValueChains })
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-2xl mx-auto pb-12">
            ...
            <Card>
                ...
                <CardContent className="space-y-4">
                    <div>
                        <label className="text-sm font-medium text-gray-700 mb-2 block">Pin Location on Map</label>
                        <Controller
                            name="location"
                            control={control}
                            render={({ field }) => (
                    ...
                    <div>
                        <label className="text-sm font-medium text-gray-700">County (Optional)</label>
                        <Controller
                            name="county"
                            control={control}
                            render={({ field }) => (
